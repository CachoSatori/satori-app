...
>
> **🆕 2026-07-09 (rama de trabajo — PREPARA, no aplica; prod/main intactos):** SPEC **FIRMADO** [docs/SPEC-propinas-efectivo-electronico.md](docs/SPEC-propinas-efectivo-electronico.md): la **cuenta por pagar de propinas se genera SOLO por la porción electrónica** (datáfono/SINPE); el **efectivo se lo queda el equipo y NUNCA genera movimiento ni pendiente**. El *take-home* por empleado NO cambia — cambia solo el MONTO de la cuenta por pagar. Rama **`feat/propinas-efectivo-electronico`** lista (build EXIT 0 · **280 tests verdes** · `tipCalculations.ts`/`cashUtils`/`computeTotals`/`posFiscal` intactos — `tipCalculations.ts` **byte-idéntico**). **Mig `046_pool_barra_electronico.sql` PENDIENTE de aplicar a STAGING con firma** (ritual: `cat supabase/.temp/project-ref` = `hwiatgicyyqyezqwldia`; `db push` sigue frenado → aplicar quirúrgico; `schema_migrations` NO se toca). Sin merge aún. Backlog → PROMPT-CONTINUACION P2 #6.
>
