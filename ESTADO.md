...
**⚠️ Reconciliación del ledger = sesión dedicada (NO tocar el historial):** ahora **los dos entornos** arrastran migraciones out-of-band (prod suma **038–045 + subset core de 026**; staging **039–046**). Persisten 009 (drift) y 035 (fantasma, solo en `propina-pool`). **`db push`/`repair` FRENADOS** hasta esa sesión. Todo idempotente. **`schema_migrations` NO se tocó** (ni en el pase ni en el hotfix).
...
1. **🔴 Reconciliación del ledger de migraciones — ahora en AMBOS entornos.** Sesión dedicada; resolver 009/035 y las out-of-band (prod **038–045 + subset core de 026**, staging 039–046). Bloquea `db push`.
2. **🔍→🖊️ Pagos pendientes en Proveedores — DIAGNOSTICADO + DECISIÓN FIRMADA (read-only 2026-07-06).** El "14" en rojo **NO son pendientes**: es `overdueCount` (proveedores activos vencidos por **ciclo**, etiqueta engañosa). Pendientes reales = **5** (3 legítimos + **2 huérfanos** `supplier_id` NULL, ₡150.043,52); **FK íntegra**. **Firmado:** el rojo = **deuda real registrada**; la agenda de ciclo va con **indicador/etiqueta aparte**; los **2 huérfanos = basura → rechazar**. Todo **post-ventana, con firma al ejecutar** (es escritura). Detalle → HALLAZGOS + PROMPT-CONTINUACION P1 #1 / P2 #5.
