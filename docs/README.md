...
1. **[../ESTADO.md](../ESTADO.md)** — foto compacta: ramas (**main `79d8004`** INTACTO en prod; **staging `eefa056`**), prod vs staging, migraciones. **§(b27) = lo de ESTA sesión (2026-06-27):** limpieza de código muerto (mergeada), borrar-día/descartar-turno por la cascada (mergeado, ✅ validado físicamente — pruebas A y B), y la foto de factura normalizada en el navegador (✅ validada + MERGEADA `eefa056`). §(b) = sesión previa (040–043 + OPCIÓN A + tests DOM). §(d) discrepancias de ledger (+044). §(g) validación física de la sesión COMPLETA; único pendiente opcional no bloqueante: verificación SQL de 0 `accounting_entries` huérfanos.
