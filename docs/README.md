...
1. **[../ESTADO.md](../ESTADO.md)** — foto compacta: ramas (**main `a0d9f0d`** en prod; **staging `bb93335`**), prod vs staging, migraciones. **§header = lo de ESTA sesión (2026-06-28):** IDOR de `extract-document` **cerrado EN PROD** (desplegado al Supabase de prod + main alineado; smoke `401`; validación física con rol de caja; pendiente OPCIONAL cross-user) + main alineado con prod + footgun del link tapado **en staging** (no en main). §(a) ramas · §(b) prod vs staging · §(c) migraciones (sin cambios) · §(d) build por módulo (sin código; Pages de main verde) · §(e) plata (sin cambios) · §(f) humanos/prolijidad. El detalle de las sesiones 2026-06-25/26/27 está archivado en `ESTADO-ARCHIVO.md`.
2. **[../PROMPT-CONTINUACION.md](../PROMPT-CONTINUACION.md)** — el PLAN: **★ PENDIENTES NUEVOS (2026-06-28):** portar el `.gitignore` de `supabase/.temp/` a main [trivial], prueba cross-user del IDOR [opcional], bumpear GitHub Actions [deuda menor]. **★ PRÓXIMO** = construcción del módulo de unificación (regenerar tipos TS → F3–F5). Pase a prod pendiente: **mig 039** (el IDOR **ya está en prod**). ⚠️ NUNCA `staging`→`main` en bloque: solo pase quirúrgico. **RITUAL del link a staging antes de cualquier comando de DB.**
