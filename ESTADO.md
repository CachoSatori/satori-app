...
> **Handoff: 2026-06-28 (continuación — CI/infraestructura). SIN código de app, SIN esquema, SIN datos.** Esta sesión subió las GitHub Actions del deploy a **Node 24 (`@v5`)** en **main y staging**, untrackeó `supabase/.temp/` también en **main** (antes solo en staging), y borró las 2 ramas de prep. **`main` = `52d1475`** · **`staging` = `3b821f0`**.
>
...
| `staging` | `3b821f0` | **Fuente de verdad del trabajo nuevo.** = `bb93335` (handoff 2026-06-28: todo `main` + PoS/KDS/comandero + Bandeja Etapa 1 + saga Realtime + durabilidad caja/outbox + auth-recovery + IDOR `c38a252` + cascada mig 039 + autorización de gerencia inline mig 044 + esquema 040–043 + tests DOM + limpieza de código muerto + borrar-día/descartar-turno por cascada + foto de factura normalizada + `.gitignore` de `supabase/.temp/`) **+ 🆕 GitHub Actions del `deploy.yml` en Node 24 (`@v5`)** (`3b821f0`, FF; el workflow **no** corre en staging — solo cierra el drift con main). `deploy.yml` **byte-idéntico a main**. |
