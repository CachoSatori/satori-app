...
6. **🗄️📊 Import histórico Excel→app — Fase A (diseño) + Fase B (import a STAGING) EJECUTADAS 2026-07-08; falta validación física + pase a prod.** Del hallazgo "convención de ventas" (HALLAZGOS 2026-07-07). **HECHO en staging** (`hwiatgicyyqyezqwldia`; **prod intacto**; todo reversible por marca `[migración 2026-07]` + backups): **10.842 `cash_movements`** (Σ₡410,47M) + tabla nueva **`ventas_efectivo_hist`** (**2.448 turnos, Σ₡274.817.196,85** exacto al Excel). Corte **parametrizado `CORTE_APP='2026-06-30'`** (el pase a prod usará otra fecha). Excluidos: **8 REVISAR + 4 tips ambiguas**. **4 suppliers mergeados** (dedupe). Verificación adversarial read-only: sumas/conteos exactos, 0 dangling. Detalle completo → **HALLAZGOS.md (2026-07-08)**. **Pendiente:** (a) **validación física del dueño en staging** (Caja→Movimientos; ventas por SQL, aún sin vista en UI); (b) decidir nuances (`caja_origen` uniforme, traspasos "Del Banco a Caja" no suman en `saldoCajaFuerte`, vista de `ventas_efectivo_hist`); (c) **Fase C = pase a PROD** — sesión aparte, otro `CORTE_APP`, con firma. **Referencia F4+ (PoS propio):** único gap real de negocio = **CXC** (mesas impagas), fuera de este import. Fuente: `MIGRACION-COMPLETA-Satori.xlsx`. TC de la casa (para reconstruir ₡ desde US$): 510 ene-24 · 500 feb–mar-24 · 490 abr-24→feb-26 · 470 mar-26→hoy; 2023 sin PoS → ₡545.
