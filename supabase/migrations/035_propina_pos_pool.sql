-- ⚠️ DDL YA APLICADO en STAGING (out-of-band). Traído desde la rama `propina-pool`
-- SOLO para alinear repo↔ledger (035 estaba en el ledger sin archivo). El CÓDIGO de la
-- feature propina-pool NO está mergeado. NO re-ejecutar: 035 ya figura APLICADO en el ledger.
--
-- 🔎 Nota de auditoría (Fase B1, 2026-07-23): el `revoke execute ... from anon` de la última
-- línea es INEFECTIVO. `create function` otorga EXECUTE a PUBLIC por defecto, y revocarle a
-- `anon` no le quita lo que hereda de PUBLIC: hoy en staging `has_function_privilege('anon',
-- 'sync_pos_tips_to_pool', 'execute')` = TRUE. El patrón correcto es el de la mig 045:
-- `revoke all on function ... from public, anon`. NO se corrige acá a propósito — este archivo
-- debe reflejar EXACTAMENTE el DDL que ya se aplicó. Corregirlo es una migración nueva.
