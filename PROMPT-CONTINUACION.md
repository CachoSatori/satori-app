...
1. **✅ CONSTRUIDO 2026-07-10 (rama `fix/proveedores-rojo-deuda-real`, SIN merge — review del dueño primero; pendiente smoke). Pagos pendientes en Proveedores** (diagnóstico read-only 2026-07-06 + firma 2026-07-09). Recordatorio del hallazgo: el **"14" en rojo NO eran pendientes** — era `overdueCount` (agenda de ciclo, etiqueta engañosa "pagos pendientes"); los **pendientes reales** son **5** en prod (3 legítimos + **2 huérfanos** `supplier_id` NULL, **₡150.043,52**; uno de 2020). **Construido (UI/lógica, sin esquema, sagrados intactos):** (a) el **rojo cuenta `pendCount`** = movimientos `status='pendiente'` (deuda real, **incluye huérfanos**) — lógica pura en `proveedoresStatus.ts`, testeada; (b) la **agenda de ciclo** (`overdueCount` → `agendaCount`) pasa a un **indicador ámbar aparte** ("N con ciclo de compra vencido"), NO rojo; (c) proveedor **'Puntual'** (nuevo valor de `ciclo_pago`, **sin migración** — es texto libre) sale de la agenda → **mata el "14"**; (d) la pestaña **Pendientes** ahora tiene **✕ Rechazar** (con **autorización de gerencia**, funciona con `supplier_id` NULL) → el dueño rechaza los 2 huérfanos él mismo en prod tras el pase (**cero SQL**). Detalle → **HALLAZGOS.md** (2026-07-06) + `REPORTE_pendientes_huerfanos_2026-07-06.md`.
...
5. **✅ CONSTRUIDO 2026-07-10 (rama `fix/proveedores-rojo-deuda-real`, sin merge; pendiente smoke) — Semántica del rojo "pagos pendientes" en Proveedores.** Firmado (2026-07-09) y ya implementado junto con P1 #1: el rojo cuenta **deuda real** (`pendCount`, `status='pendiente'`); la **agenda de ciclo** (`overdueCount`) es un **indicador ámbar aparte**; proveedor **'Puntual'** sale de la agenda (sin migración). Ver P1 #1 para el detalle de lo construido y el flujo de rechazo de huérfanos. Detalle → HALLAZGOS.md + `REPORTE_pendientes_huerfanos_2026-07-06.md`.
