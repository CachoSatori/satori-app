...
>
> **🆕 2026-07-09 — ✅ Propinas efectivo/electrónico EN STAGING (validado en piso):** la cuenta por pagar es **solo lo electrónico**. Mig 046 aplicada a staging out-of-band (ledger intacto), rama **mergeada** (`staging` = `2a0852e`), **smoke ✓**. **Pendiente = pase a PROD** (post-ventana, con firma; incluye aplicar la 046 a prod). Detalle → **P2 #6** + [docs/SPEC-propinas-efectivo-electronico.md](docs/SPEC-propinas-efectivo-electronico.md).
...
6. **✅ Propinas efectivo vs electrónico — EN STAGING, VALIDADO FÍSICAMENTE (2026-07-09). Pendiente: PASE A PROD (post-ventana, con firma).** La **cuenta por pagar de propinas se genera SOLO por lo electrónico** (datáfono/SINPE); el **efectivo se lo queda el equipo, nunca genera movimiento ni pendiente**. El reparto/`take_home` por empleado NO cambia (solo el monto de "Propinas por pagar"). **Hecho en staging:** mig **046** aplicada out-of-band (Management API `db query --linked`; `schema_migrations` intacto); rama `feat/propinas-efectivo-electronico` **mergeada** (`staging` = `2a0852e`); **smoke en piso ✓** (distribución idéntica al modelo viejo · solo el electrónico genera "por pagar" · turno solo-efectivo no genera nada). Sagrados intactos (`tipCalculations.ts` byte-idéntico). **Falta para PROD (con firma, post-ventana):** aplicar la **046 a PROD** (ref `yiczgdtirrkdvohdquzf`; ritual del link + Management API; `db push` frenado → quirúrgico; `schema_migrations` intacto) **ANTES** de pasar el código (si el código va primero, `getTipPayoutsSince` rompe pidiendo una columna que no existe) + pasar el código (con el gran pase o quirúrgico). Distinto de `propina-pool` (P2 #2, la pregunta pool-único-vs-separado). SPEC → [docs/SPEC-propinas-efectivo-electronico.md](docs/SPEC-propinas-efectivo-electronico.md).
