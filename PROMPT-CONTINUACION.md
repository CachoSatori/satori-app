...
>
> **🆕 Sesión 2026-07-06 → 09 (solo estabilización + docs, CERO código):** ✅ tokens GitHub rotados (P1 #3); 🖊️ pagos "huérfanos" reencuadrados con **decisión FIRMADA** (P1 #1 + P2 #5, listos para implementar post-ventana); ✅ "convención de ventas" CERRADA (HALLAZGOS); 🛑 **migración del histórico ejecutada y CANCELADA** en staging (P1 #6, revertida quirúrgico — prod intacto); nuevo P1 #7 "arranque limpio de prod"; nuevo hallazgo `saldoCajaFuerte` sin ancla. **Cola de implementación post-ventana (todo con firma):** semántica del rojo + huérfanos (P1 #1/P2 #5) · reconciliación del ledger (P1 #2) · hora-CR (P1 #4) · arranque limpio (P1 #7).
>
> **🆕 2026-07-09 — ✅ DOS features EN STAGING, validadas en piso (`staging` = `c26df15`):** (i) **propinas efectivo/electrónico** (la cuenta por pagar es solo lo electrónico; mig 046 aplicada a staging out-of-band) y (ii) **cierre del día: ventas ₡0 con confirmación + resumen previo** (sin esquema). **Pendiente = pase a PROD post-ventana con firma, las dos JUNTAS** — orden: **mig 046 a prod PRIMERO, código después**. Detalle → **P2 #6** + [docs/SPEC-propinas-efectivo-electronico.md](docs/SPEC-propinas-efectivo-electronico.md).
...
1. **🔍→🖊️ Pagos pendientes en Proveedores — DIAGNOSTICADO + DECISIÓN FIRMADA (read-only 2026-07-06; firma 2026-07-09).** El **"14" en rojo NO son pendientes:** es `overdueCount` (`CashProveedores.tsx:83`) — proveedores **activos** vencidos por **ciclo de pago**, con etiqueta engañosa "pagos pendientes". Los **pendientes reales** (`status='pendiente'`) son **5** en prod: 3 legítimos + **2 huérfanos** (`supplier_id` NULL, **₡150.043,52**; uno de 2020); **FK íntegra, 0 dangling.** **DECISIÓN FIRMADA (implementar post-ventana, con firma al ejecutar):** (a) el **rojo = deuda real registrada** (`status='pendiente'`), no la agenda; (b) la **agenda de ciclo** va con **indicador/etiqueta aparte** (corrige `CashProveedores.tsx:75-83`; evaluar ciclo **'Puntual'**/dormido — ver P2 #5); (c) los **2 huérfanos = basura → rechazar** (escritura). **NO accionar en caliente** (estabilización). Detalle → **HALLAZGOS.md** (2026-07-06) + `REPORTE_pendientes_huerfanos_2026-07-06.md`.
...
5. **✅🖊️ Semántica del rojo "pagos pendientes" en Proveedores — DECIDIDO (2026-07-09), implementar post-ventana.** Firmado: el rojo cuenta **deuda real registrada** (`status='pendiente'`); la **agenda de ciclo** (vencidos por ciclo, hoy `overdueCount`) pasa a un **indicador/etiqueta aparte** (no "pagos pendientes"). Implementación (corrige `CashProveedores.tsx:75-83`; evaluar ciclo **'Puntual'**/estado dormido para one-off) va junto al rechazo de los 2 huérfanos (P1 #1). Ya no espera decisión — espera **ventana + firma al ejecutar**. Detalle → HALLAZGOS.md + `REPORTE_pendientes_huerfanos_2026-07-06.md`.
6. **✅ EN STAGING, VALIDADAS EN PISO (2026-07-09) — DOS features van JUNTAS al MISMO pase a prod (post-ventana, con firma). `staging` = `c26df15`.** Sagrados intactos en ambas (`tipCalculations.ts` byte-idéntico; `cashUtils`/`computeTotals`/`posFiscal` sin tocar).
    - **(i) Propinas efectivo/electrónico** (rama `feat/propinas-efectivo-electronico`, mergeada): la cuenta por pagar de propinas se genera **SOLO por lo electrónico** (datáfono/SINPE); el efectivo se lo queda el equipo, nunca genera movimiento ni pendiente; el reparto/`take_home` no cambia. **Tiene esquema:** mig **046** (`tip_sessions.pool_barra_electronico_crc`) aplicada a STAGING out-of-band. Smoke ✓ (distribución idéntica · solo electrónico genera "por pagar" · solo-efectivo no genera nada). SPEC → [docs/SPEC-propinas-efectivo-electronico.md](docs/SPEC-propinas-efectivo-electronico.md).
    - **(ii) Cierre del día — ventas ₡0 + resumen** (rama `feat/cierre-ventas-cero-y-resumen`, mergeada): cerrar con ventas ₡0 **solo con confirmación explícita** (campo vacío bloquea); **modal de resumen** antes de confirmar. **SIN esquema** (solo UI/gates; el persistido no cambió). Smoke ✓ 4/4 (cierre normal desde el modal · ₡0 con/sin check · vacío bloquea · noche solo-dólares cierra).
    - **ORDEN DEL PASE A PROD (con firma del dueño):** **1º** aplicar la **mig 046 a PROD** (ref `yiczgdtirrkdvohdquzf`; ritual del link + Management API; `db push` frenado → quirúrgico; `schema_migrations` intacto) — el código de propinas la necesita o `getTipPayoutsSince` rompe pidiendo una columna que no existe; **2º** el **código** después (las dos features viajan juntas; con el gran pase o quirúrgico). El cierre no necesita esquema pero va en el mismo código. Distinto de `propina-pool` (P2 #2, la pregunta pool-único-vs-separado).
