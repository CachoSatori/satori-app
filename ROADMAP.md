...
> **🆕 Sesión 2026-06-28 (cont. — CI/infra, sin código de app):** `main` = `52d1475` · `staging` = `3b821f0`. (1) **GitHub Actions del `deploy.yml` subidas a Node 24 (`@v5`)** en main (`52d1475`, deploy verde, warning de Node 20 desaparecido) y staging (`3b821f0`); `deploy.yml` **byte-idéntico entre ambos** (drift cerrado). **No** se tocó `node-version: 20` del build (Node 22 = cambio aparte). (2) **`supabase/.temp/` untrackeado + ignorado también en main** → clon fresco de main ya no arranca enlazado a prod. (3) 2 ramas de prep integradas por FF y borradas. La sesión previa **2026-06-28 (IDOR de `extract-document` cerrado en prod + main alineado + footgun en staging)** y la **2026-06-27** están archivadas en `ESTADO-ARCHIVO.md`. Ver ESTADO §header + §(a).
