...
> **🆕 Sesión 2026-06-28 (cont. — CI/infra + 2 fixes de plata client-side, ambos ya en PROD):** `main` = `a14da50` · `staging` = `ec70598`. (1) **GitHub Actions del `deploy.yml` subidas a Node 24 (`@v5`)** en main (`52d1475`, deploy verde, warning de Node 20 desaparecido) y staging (`3b821f0`); `deploy.yml` **byte-idéntico entre ambos** (drift cerrado). **No** se tocó `node-version: 20` del build (Node 22 = cambio aparte). (2) **`supabase/.temp/` untrackeado + ignorado también en main** → clon fresco de main ya no arranca enlazado a prod. (3) 2 ramas de prep integradas por FF y borradas. **(4) 🆕 2 fixes de plata client-side a STAGING:** Hallazgo B (drain del outbox en `SIGNED_IN`, `492eaa5`) y la **estabilización del render de Propinas** (`ec70598`: parpadeo de Take Home + picker de coberturas) — 🆕 **ambos YA EN PROD** (`a14da50`, cherry-pick selectivo; deploy verde, `version.json.commit=a14da50`) + validados en staging, smoke en prod pendiente del OK de la dueña; `calcTurno`/sagrados byte-idénticos, sin migración; ver tabla de fases. La sesión previa **2026-06-28 (IDOR de `extract-document` cerrado en prod + main alineado + footgun en staging)** y la **2026-06-27** están archivadas en `ESTADO-ARCHIVO.md`. Ver ESTADO §header + §(a).
