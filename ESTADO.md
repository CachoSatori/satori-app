...
> 2. **Borrar-día y descartar-turno ahora pasan por la CASCADA — MERGEADO a staging** (`b8ab78c`, fast-forward): `discardDiaCompleto` y `discardCashSession` (`src/shared/api/cash.ts`) borraban `cash_movements` con `.delete()` crudo → salteaban `delete_movement_cascade` (mig 039/044) → dejaban `accounting_entries` huérfanos (sin reversa) e `inventory_review_task` colgadas, sin auditoría. Ahora borran **cada** movimiento por el RPC (con credenciales de gerencia de mig 044), respetando el orden **movimientos→cierre→sesiones**; error parcial recuperable, no tragado. Test nuevo `cash.discardDia.test.ts`. **👁️ Falta validación física:** borrar un día con factura de mercadería y confirmar 0 `accounting_entries` huérfanos ni tareas colgadas.
...
| **🆕 Borrar-día / descartar-turno por la cascada** (no dejan asientos huérfanos) | 🟢 verde · 👁️ falta validación física | **staging** (`b8ab78c`) |
...
- **🆕 👁️ VALIDACIÓN FÍSICA pendiente (en staging):** **borrar un día con factura de mercadería** y confirmar que NO quedan `accounting_entries` huérfanos ni `inventory_review_task` colgadas (§b27.2, en staging `b8ab78c`). *(La foto de factura normalizada §b27.3 ya fue ✅ validada y mergeada — `eefa056`.)*
