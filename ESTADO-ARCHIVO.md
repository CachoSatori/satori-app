...
Acta completa → [PASE-POZO-A-PROD.md](PASE-POZO-A-PROD.md). Diagnóstico y harness →
[scripts/t0-reconciliacion-cajas/](scripts/t0-reconciliacion-cajas/README.md). Actas de staging →
[CORTE-2026-07-22.md](scripts/refresh-staging/CORTE-2026-07-22.md) ·
[TRIAGE-TARJETA.md](scripts/refresh-staging/TRIAGE-TARJETA.md) ·
[ARRANQUE-CERO.md](scripts/refresh-staging/ARRANQUE-CERO.md).

Resumen: modelo de **pozo único de efectivo** (todo movimiento físico afecta un solo saldo, una sola
vez), **corte hacia adelante** en `2026-07-22`, **asiento de arranque** `296d032d` (₡744.570/$3.441,
el único write autorizado a prod), **paginación del fetch con desempate por `id`** (el `select` sin
paginar se comía filas en silencio al pasar el `max_rows` del proyecto), tarjeta de Movimientos al
pozo post-corte, filtro `DESDE`=corte por defecto (histórico intacto), exclusión del asiento de
arranque de "Ingresos (período)", y los dos fixes de pantalla de CashTurno. **Cero migraciones ·
sagrados byte-idénticos.** El **primer cierre real bajo el pozo (22/07) CUADRÓ**.

## 🗄️ Entradas de pase 2026-07-09 → 2026-07-21 (movidas del preámbulo de ESTADO.md)
...
**La cadena (orden de aplicación a staging):**
1. **`a4b1be3` — Cierre del Día: resumen en filas** (rediseño visual patrón Caja Diaria; cero lógica).
2. **`4fddc5e` + `46ab5c6` — Verificación del cierre en columnas ₡ | US$ + FÓRMULA USD FIRMADA.** `calcDeberiaUSD` ahora **incluye `saldoBase.usd`** → el cuadre detecta **retiros USD sin registrar** (antes eran invisibles: la fórmula vieja solo miraba ventas). Tema claro cálido en toda la pestaña Cierre. Test `CashCierre.deberiaUSD.test.ts` cubre el caso. **Este cambio destapó una deuda histórica: −$2678 en el ledger de CF de staging** (espejo de prod).
3. **`86739f5` — Autorización de gerencia SOLO POR CONTRASEÑA · mig 045 FIRMADA + APLICADA a staging.** `verify_manager_password(p_password)` (SECURITY DEFINER, `search_path=''`, anti fuerza-bruta con `pg_sleep(0.5)` en el fallo, **rechaza colisiones** de contraseña con error explícito). La contraseña **identifica** al manager que autoriza (devuelve su identidad). Reemplaza el modal viejo email+password de `ManagerOverride`. **HALLAZGO clave:** la edición de un pago **NO es un UPDATE** sino un **`delete_movement_cascade` + `persistPago` recreate** → el bloqueo real de "editar como cajero" estaba en el **reenvío de credenciales al path de plata** (la RPC de borrado re-valida server-side). Por eso la 045 y el envolver el edit-path con `requireManager`. mig 045 = `045_verify_manager_by_password.sql`, aplicada a staging con `db query --linked` (fuera de `schema_migrations`, patrón 044).
4. **`a628dbf` — Asistente "➕ Agregar" reordenado** (Foto → Clasificación → Proveedor → Montos → Descripción → Fecha) + **nota "⚠ sin foto"** en pagos de mercadería manuales sin factura enlazada (control del manager; deriva de `listLinkedDocs`).
5. **`d5c8138` — Foto de factura en el detalle de Revisión + paso de CONFIRMACIÓN al adjuntar.** Al adjuntar una foto a una tarea sin factura: se muestra proveedor leído vs proveedor del pago, total leído vs monto (con aviso de descuadre 2%/₡50); **Cancelar no crea nada** (la foto queda huérfana en el bucket, sin fila); el pago **queda intacto** (verificado por test).
6. **`2784d93` — Panel lateral de la foto en desktop (≥900px) con zoom + drag-to-pan.** Reusa la signed URL ya cargada del padre (no re-firma). Móvil (<900px) sigue con thumb + fullscreen como estaba.
7. **`e959fe5` — OPCIÓN B FIRMADA: el cierre con diferencia crea automáticamente "Ajuste de cierre" en Caja Fuerte.** Faltante → egreso (resta), Sobrante → ingreso (suma), en **ambas monedas**. `client_op_id` **determinístico** (SHA-256 de `cierre-ajuste-{fecha}-{moneda}`) → re-confirmar el mismo cierre **no duplica**. Deshacer el cierre **borra el ajuste**. El `saldoBase` del propio día excluye el ajuste de esa fecha (anti-doble-conteo, mismo tratamiento que 'Ventas cierre'). Subcategoría **"Reajuste"** en Movimientos para cuando la plata aparece/falta después. **Gate USD** agregado: el motivo obligatorio ahora cubre las dos monedas (`cierreNecesitaAjuste`).
8. **`a0e361c` — Flujo guiado del asistente:** al abrir, dos opciones grandes — **📷 Sacar foto (protagonista)** / **Carga manual**. Cancelar la cámara vuelve a las opciones (no a un form vacío). autoFocus resuelto por diseño (Proveedor en modo manual).
9. **`380cb9a` — PROPINAS POR LA VÍA REAL (FIRMADO): el campo tipeado de propinas del cierre MURIÓ.** Ahora el encargado ve los turnos de propinas pendientes y paga con **"💵 Pagar ahora"** vía el `pagarPropina` existente (helper compartido `propinaPago.ts`, misma vía que Caja Diaria). La matemática del cierre resta propinas **EFECTIVAMENTE PAGADAS** (movimientos del libro), no un tipeo. **El FALTANTE FANTASMA quedó enterrado** (el gap venía de que 'Ventas cierre' ingresaba el BRUTO a CF mientras el "debería" restaba propinas tipeadas). **El test del gap se DIO VUELTA:** antes documentaba `ledger = contado + propinas`, ahora **AFIRMA `ledger = contado` exacto**. Deshacer el cierre NO borra pagos de propinas (plata entregada); al re-cerrar se recalcula neto e idempotente. Sin migración (`propinas_m/n_crc` reutilizados por compat con históricos/KPIs).
10. **`ddb1c08` — Rediseño de las KPI cards de Caja al tema claro cálido** (`.cd-saldo-*`, clase compartida → afecta las 4 pestañas: Movimientos, Pendientes, Proveedores, Resumen) + card "Ajustes de cierre" muestra **₡ y US$** + diferencias discriminadas por moneda en el bloque rojo del cierre y en la tarjeta de "Día cerrado".

**FUERA DEL REPO (cambio de infra, esta sesión):** el secret **`ANTHROPIC_MODEL`** de la Edge Function `extract-document` se cambió en **staging** de `claude-haiku-4-5` a **`claude-sonnet-4-5`** (`supabase secrets set --project-ref hwiatgicyyqyezqwldia`). Ismael verificó con facturas difíciles: **mejora notoria en lectura y confianza.** Es una env var — sin código, reversible al instante. **Pendiente replicar en prod.**

**SINCERAMIENTO USD DE CAJA FUERTE (hecho en staging, pendiente prod):** la fórmula USD firmada (pase 2) destapó −$2678 histórico en el ledger de CF de staging (espejo de prod). Ismael cargó el ajuste inicial en staging (Movimientos → Ingreso · Otro CF, USD = físico + 2678, ₡ = 0). En prod hay que repetir con el conteo físico del día del pase.

**Cambio de ritual (guardrail pre-comando-de-base):** el chequeo pasa a `supabase/.temp/project-ref` (ya **no existe** `linked-project.json` en el CLI v2.105). Debe decir `hwiatgicyyqyezqwldia` para staging antes de cualquier comando de base.
