...
>
> **🔥 2026-07-17 — HOTFIX (afecta PROD): el buscador de Movimientos crasheaba la pantalla al tipear.** Vehículo: rama **`hotfix/movimientos-buscar-null`** sobre `staging` (`d85453a`) — **sin merge, espera revisión del asesor**. **Síntoma:** en Caja → Movimientos, tipear cualquier cosa en "Buscar..." tiraba la pantalla al **ErrorBoundary**. **Causa raíz — bug LATENTE de datos, NO de los pases del 16:** `CashMovimientos.tsx` llamaba `.toLowerCase()` **directo** sobre `m.supplier_name`/`m.employee_name`. En la base esos 2 campos son **NULLABLE** (`supabase.gen.ts`: `string | null`) y hay filas viejas con `null`, pero **`database.ts` los declaraba `string` a secas → el tipo MENTÍA y TS nunca avisó**. El camino de lectura (`getCashMovements`) castea la fila cruda (**`data as CashMovement[]`**), así que el `null` llega intacto al componente; los caminos de **escritura** sí coercionaban (`?? ''`), por eso solo revientan los datos **viejos**. **Staging no crasheaba solo porque su espejo de datos es viejo** — el código era igual de vulnerable. **Fix (3 partes, solo código):** (1) filtro **null-safe** (`(m.supplier_name ?? '').toLowerCase()`, ídem `employee_name` y `description` —defensivo, aunque figure NOT NULL—); (2) **el tipo dice la verdad**: `supplier_name`/`employee_name` → **`string | null`** en `database.ts` (previene la reincidencia: ahora TS marca el uso inseguro); (3) la **cascada del tipo fue mínima — 1 solo sitio** (`CashMovimientos.tsx:447`, `defaultValue` de un input, `null || null` → `null`; cerrado con `|| ''`) → **cero lógica de plata tocada**. **Grep general: 0 sitios inseguros restantes** — todos los demás lectores (`CashPendientes:45`, `InboxModule:346`, `CashTurno`, `FinanzasModule`) ya usaban `||`/`??`. Gates verdes: **test nuevo** (`CashMovimientos.buscarNull.test.tsx`, 3 casos; **verificado que falla sin el fix** con el `TypeError: Cannot read properties of null (reading 'toLowerCase')` exacto de prod) · **299 tests sin env** · `VITE_APP_ENV=production npm run build` **EXIT 0** · **sagrados intactos** (diff vacío) · **CERO esquema**. Rollback: revert del commit + redeploy (sin migración → trivial). **➡️ Circuito completo hoy: review del asesor → staging → prod.**
>
...
2. **✅ EN PROD 2026-07-16 (`main` = `7377240`) — Proveedores: el rojo = deuda real.** El "14" en rojo **NO eran pendientes**: era `overdueCount` (agenda de ciclo, etiqueta engañosa). Pasó a prod (UI/lógica, sin esquema, sagrados intactos): el **rojo cuenta `pendCount`** (deuda real `status='pendiente'`, incluye los **2 huérfanos** `supplier_id` NULL de ₡150.043,52); la **agenda de ciclo** quedó como **indicador ámbar aparte**; proveedor **'Puntual'** sale de la agenda (mata el "14"); la pestaña **Pendientes** permite **rechazar** (con gerencia, anda con `supplier_id` NULL) → el dueño rechaza los 2 huérfanos en prod. **✅ SALDADO (smoke del dueño HECHO 2026-07-16):** rechazó los 2 huérfanos desde la UI en prod (cero SQL) → **pestaña Pendientes 5→3**, validado en piso. (El "que el rojo diga 5" quedó *moot*: el 2º pase del mismo día retiró el badge rojo de Proveedores; la confirmación vino por Pendientes.) Este punto queda **cerrado**.
    **🆕 2026-07-16 — SIMPLIFICACIÓN FIRMADA por el dueño tras verlo en prod** (rama `fix/proveedores-simplificar`, sobre staging, **SIN merge** — review del asesor primero): la pestaña **Proveedores = SOLO la lista de proveedores**. Se **eliminan** el badge rojo de pendientes + su panel (**duplicado**: la pestaña Pendientes ya avisa con su propio `cd-pend-badge`), el chip ámbar de agenda y los banners de "Agenda de compra". **La info no se pierde: vive en la tarjeta de cada proveedor** (su deuda `pendingCRC`, su ciclo —incluido **'Puntual'**—, último/próximo pago). `CashPendientes` **intacto** (Rechazar sigue). Al quedar sin uso se borraron `contarAgenda`/`contarPendientes`/`totalPendienteCRC` y sus tests; `computeSupplierStatus`/`esProveedorPuntual` siguen (los usan las tarjetas). Detalle → PROMPT-CONTINUACION P1 #1 / P2 #5.
