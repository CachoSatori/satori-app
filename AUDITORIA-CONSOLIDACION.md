...
### 1.2 PoS — NO hay control de concurrencia optimista
Las escrituras del PoS son last-write-wins; no hay `version`/`If-Match`. Casos reales con 2 tablets:
- **Doble cobro de la misma mesa** — ✅ **RESUELTO en staging (mig 033, rama `fix-doble-cobro`),
  pendiente de pase a prod**. Antes: `INSERT pos_payments` se ejecutaba aunque el `UPDATE` de cierre
  fuera idempotente → 2 filas de pago. Ahora: `client_op_id UNIQUE` parcial (colapsa el reenvío del
  mismo intento) + RPC SECURITY DEFINER atómica `pos_cobrar_orden`/`pos_cobrar_check` con `FOR UPDATE`
  + precondición ("Esta cuenta ya fue cobrada"). La matemática NO se tocó: la RPC solo persiste.
  Verificado: doble-tap → 1 fila; otra caja → error; split → N filas legítimas
  (`scripts/test-cobro-idempotente.py`, 10/10).
...
- **Top para decisión humana** (actualizado):
  1. ✅ ~~Doble cobro concurrente~~ → RESUELTO en staging (mig 033); falta pase a prod.
  2. 🔴 `rules-of-hooks` en `MiRendimiento.tsx` (bug latente del módulo viejo).
  3. 🟡 Atomicidad de merge/reopen (mismo patrón RPC que el cobro) + pausar refetch con modal abierto.
