...
>
> **🆕 Sesión 2026-07-06 → 09 (solo estabilización + docs, CERO código):** ✅ tokens GitHub rotados (P1 #3); 🖊️ pagos "huérfanos" reencuadrados con **decisión FIRMADA** (P1 #1 + P2 #5, listos para implementar post-ventana); ✅ "convención de ventas" CERRADA (HALLAZGOS); 🛑 **migración del histórico ejecutada y CANCELADA** en staging (P1 #6, revertida quirúrgico — prod intacto); nuevo P1 #7 "arranque limpio de prod"; nuevo hallazgo `saldoCajaFuerte` sin ancla. **Cola de implementación post-ventana (todo con firma):** semántica del rojo + huérfanos (P1 #1/P2 #5) · reconciliación del ledger (P1 #2) · hora-CR (P1 #4) · arranque limpio (P1 #7) · **🔬 research quick-wins C1/C2/C3 (P1 #8, FIRMADOS 2026-07-10)**.
>
> **🆕 2026-07-10 — ✅ PASE A PROD COMPLETO Y VALIDADO EN PISO.** Las 2 features ya están en producción: (i) **propinas efectivo/electrónico** (la cuenta por pagar es solo lo electrónico) y (ii) **cierre del día: ventas ₡0 con confirmación + resumen previo**. Ejecutado en orden: mig 046 a prod out-of-band + merge FF a `main` (`6c65f25`) + deploy + smoke en piso ✓. **Salen de la cola.** Queda la reconciliación del ledger (prod 038–046 + subset 026; staging 039–046; P1 #2). Detalle → **P2 #6** + [docs/SPEC-propinas-efectivo-electronico.md](docs/SPEC-propinas-efectivo-electronico.md).
...
1. **✅ EN PROD 2026-07-16 (`main` = `7377240`) — Proveedores: el rojo cuenta DEUDA REAL** (diagnóstico read-only 2026-07-06 + firma 2026-07-09). Recordatorio del hallazgo: el **"14" en rojo NO eran pendientes** — era `overdueCount` (agenda de ciclo, etiqueta engañosa "pagos pendientes"); los **pendientes reales** son **5** en prod (3 legítimos + **2 huérfanos** `supplier_id` NULL, **₡150.043,52**; uno de 2020). **En prod (UI/lógica, sin esquema, sagrados intactos):** (a) el **rojo cuenta `pendCount`** = movimientos `status='pendiente'` (deuda real, **incluye huérfanos**) — lógica pura en `proveedoresStatus.ts`, testeada; (b) la **agenda de ciclo** (`overdueCount` → `agendaCount`) quedó como **indicador ámbar aparte**, NO rojo; (c) proveedor **'Puntual'** (valor de `ciclo_pago`, **sin migración** — es texto libre) sale de la agenda → **mata el "14"**; (d) la pestaña **Pendientes** tiene **✕ Rechazar** (con **autorización de gerencia**, funciona con `supplier_id` NULL).
    **⏳ SMOKE DEL DUEÑO PENDIENTE en prod:** (1) que el badge diga **5** y no 14 y coincida con la pestaña Pendientes; (2) marcar un one-off (MUSICOS, Coca) como **Puntual**; (3) **rechazar los 2 huérfanos desde la UI** — Distribuidora Isleña 2020-07-09 ₡74.126,92 y GRUPO PAMPA 2026-07-06 ₡75.916,60 (**cero SQL**).
    **🆕 2026-07-16 — SIMPLIFICACIÓN FIRMADA (rama `fix/proveedores-simplificar`, sobre staging, SIN merge — review del asesor primero).** Tras ver el fix en prod, el dueño decidió que la pestaña **Proveedores sea SOLO la lista de proveedores**. **Eliminado de `CashProveedores.tsx`:** el badge rojo "N pendientes por pagar" + su panel "Deuda pendiente registrada" (`showPending`) — **es un duplicado**: la pestaña Pendientes, al lado, ya notifica con su propio badge (`cd-pend-badge`, `CashModule.tsx:135`) —; el chip ámbar "N con ciclo de compra vencido"; y los banners de "Agenda de compra". **Conservado:** la lista/tarjetas con agregar/editar/desactivar tal cual, y **dentro de cada tarjeta** su deuda (`pendingCRC`), su ciclo acordado (**incluida 'Puntual'** y su semántica), último/próximo pago y total pagado. **`CashPendientes` intacto** (Rechazar y todo lo demás). **Sin código muerto:** `contarAgenda`/`contarPendientes`/`totalPendienteCRC` quedaron sin uso (nadie más los importaba) → borrados con sus tests; `computeSupplierStatus`/`esProveedorPuntual` siguen porque los usan las tarjetas. UI-only, cero esquema, sagrados byte-idénticos. Detalle → **HALLAZGOS.md** (2026-07-06) + `REPORTE_pendientes_huerfanos_2026-07-06.md`.
...
8. **🔬 Research quick-wins (FIRMADOS 2026-07-10, firma al ejecutar) → [docs/research/03-GAP-ANALYSIS.md](docs/research/03-GAP-ANALYSIS.md).** **C2** historial over/short del cierre + **C3** email del cierre al owner (un solo lote de reportes) · **C1** tips-owing por empleado. **Al diseñar el PILAR:** incorporar **C4/C5/B1**. **FE-CR = sesión dedicada propia (bloquea F3).** Descartes **D1–D4** registrados (no reabrir sin firma).
    **🆕 2026-07-17 — C2 + C3 CONSTRUIDOS EN STAGING (rama `feat/cierre-quickwins` sobre `staging` `e53a17a`, SIN merge — review del asesor primero).** **SIN esquema, sagrados intactos, cero migraciones.**
    - **C2 — historial de sobrantes/faltantes** (`cierreStats.ts` puro + sección nueva en `CashResumen`, respeta su filtro por mes): lista por cierre completo (fecha · diferencia ₡ con signo/color sobrante/faltante/cuadró · ajuste tipo/motivo) + agregados (cuántos cuadraron vs no · neto · absoluto). **Display de lo ya sellado en `cash_cierres_dia`, NO recalcula.** NULL-safe (`diferencia_crc`/`ajuste_*` son nullable en la base). Test: `cierreStats.test.ts` (7 casos, incluye fixture null).
    - **C3 — email del cierre al owner** (patrón Square): al confirmarse el cierre COMPLETO, `CashCierre` dispara `sendCierreEmail(cierreId)` **fire-and-forget** (si el email falla, el cierre NO se rompe). Edge Function nueva **`cierre-email`** (patrón Resend de monthly-report) **CON el fix del hallazgo #2: EXIGE `Authorization` (JWT verificado con `getUser`) y relee la fila server-side con el token del usuario (RLS = portón), NO service_role** — mismo patrón que `extract-document` post-IDOR. HTML = mismo resumen del popup (ventas M/N, propinas pagadas, distribución, total contado, diferencia/ajuste). **✅ Deployada a STAGING** (`supabase functions deploy cierre-email --project-ref hwiatgicyyqyezqwldia`; ACTIVE v1).
    - **⚠️ BLOQUEANTE DEL SMOKE DE C3:** **`RESEND_API_KEY` NO existe como secret en STAGING** (verificado con `secrets list`; en prod SÍ está por monthly-report). Sin la key la función devuelve `{ok:false, 'RESEND_API_KEY no configurada'}` y el email hace no-op silencioso (el cierre no se afecta) — pero **el email no se enviará hasta cargarla**. El dueño/asesor debe `supabase secrets set RESEND_API_KEY=… --project-ref hwiatgicyyqyezqwldia` antes del smoke. **PROD: la función va en el pase con firma — NO se tocó prod.**
    - Gates: **build prod EXIT 0** · **311 tests sin env** (299 + 12: 7 C2 + 5 buscador prov) · **diff de sagrados VACÍO** · cero migraciones.
    - **➕ EXTRA del mismo lote (pedido del dueño): buscador en Proveedores** — input en vivo en `CashProveedores` que filtra por nombre/categoría/contacto, **null-safe desde el día uno** (category/contact son nullable), estado vacío "Sin coincidencias". Test: `CashProveedores.buscar.test.tsx` (5 casos, fixture null).
