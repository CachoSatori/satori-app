...
// ── Estado local: hasta qué `pos_id` se confirmó ────────────────────────────────
// Vive en la PC del local (no en el repo). Si se pierde, el agente arranca de 0 y
// reingesta el histórico: el `(local, pos_id)` de la base hará que eso NO duplique.
 ...
 */
export function advanceLastId(actual: number, ventas: VentaCruda[]): number {
  let mayor = actual
  for (const v of ventas) if (v.posId > mayor) mayor = v.posId
  return mayor
}
