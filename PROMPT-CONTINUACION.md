...
6. **🆕🖊️📊 Import histórico completo Excel→app (POST-VENTANA de estabilización).** Del hallazgo "convención de ventas en datos migrados" (**HALLAZGOS 2026-07-07**): centralizar TODO el histórico de ventas (2023 → hoy) dentro de la app, **reemplazando** los datos actuales (son reemplazables: hay doble registro durante la estabilización). **Diseño requerido antes de tocar nada:** (a) **corte de fecha** — hasta dónde manda el histórico migrado y desde dónde el registro vivo de la app (evitar solapamiento/doble conteo); (b) **estrategia de reemplazo** (truncar+recargar `ventas_hist`/`ventas_dias` vs. upsert idempotente por fecha); (c) **reconstrucción del ₡ real desde los US$ confiables** con el TC implícito de la casa (**510** ene-24 · **500** feb–mar-24 · **490** abr-24→feb-26 · **470** mar-26→hoy; **2023 sin PoS → TC ₡545**); (d) **STAGING primero**, validar contra **cierres de referencia** conocidos antes de prod; (e) **firma del dueño**. **Referencia F4+ (PoS propio):** el único gap real de negocio es **CXC** (mesas impagas) — no es parte de este import, pero anotarlo para el diseño del PoS. Fuente: `MIGRACION-COMPLETA-Satori.xlsx` (en poder del dueño) + HALLAZGOS 2026-07-07.
